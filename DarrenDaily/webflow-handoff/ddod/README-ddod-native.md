...
- **Six curated runs**, 8 episodes each, 43 to 53 minutes. Tapping a card expands
  its episode list and makes the run the queue, so it plays straight through.
...
`ddod-player-v1.js` is uploaded as a **site asset** and loaded with `defer`:

```
https://cdn.prod.website-files.com/6a66d7a6f9d116b514a13ae1/6a9c912f32d2771126528007_ddod-player-v1.js
```

15.7 KB, of which ~6.9 KB is the episode and run data. Hosting it rather than
...
- **Six podcast app links are placeholders for Spotify, iHeart and Amazon Music.**
  Apple, YouTube and the Libsyn RSS URL are real. Confirm the other three before launch.
- The episode and run data is baked into the script. `DarrenDaily/refresh-ddod-data.py`
