(function(){
  ...
  /* ---------- credibility ribbon ----------------------------------------
     Nothing to do here any more. The ribbon is now the "Author Cred Row"
     component, which hard-codes BOTH seal sets in its own markup and
     carries its own embed for the keyframes, so it works on pages that
     never load this script. The clone that used to live here would now
     double an already-complete track. Do not reinstate it. */
  ...
  /* HubSpot ships the Role select with a bare "Role" placeholder while the two
     ...
     inconsistent. Runs again after a beat because HubSpot re-renders the
     field group once validation attaches. */
  function labelRole(){
    if(!mount) return;
    var sel=mount.querySelector('select[name="company_role"]');
    if(!sel||!sel.options.length) return;
    var first=sel.options[0];
    if(first.value==='') first.text='Your Role*';
  }
  ...
  function mountForm(){
    ...
    s.onload=function(){ if(window.hbspt) hbspt.forms.create({
      ...
      onFormReady:function(){ labelRole(); setTimeout(labelRole,400); setTimeout(labelRole,1500); } }); };
