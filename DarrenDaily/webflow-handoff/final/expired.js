...
// Nav: transparent at the very top, flips to solid white after a small scroll
  ...
  (function () {
    ...
    var POST_BASE = 'https://dd.darrenhardy.com/';       // session post URL base (matches the hardcoded hrefs)
    ...
    function parseFeed(html) {
      ...
      return Array.prototype.slice.call(doc.querySelectorAll('.sf-row')).map(function (row) {
        function txt(sel) { var el = row.querySelector(sel); return el ? el.textContent.trim() : ''; }
        var img = row.querySelector('.sf-thumb') || row.querySelector('img');
        return {
          slug:  txt('.sf-slug'),
          title: txt('.sf-title'),
          thumb: img ? (img.getAttribute('src') || '') : '',
          pub:   parseDate(txt('.sf-pub')),
          exp:   parseDate(txt('.sf-exp'))
        };
    ...
    fetch(FEED_URL, { credentials: 'same-origin' })
      ...
      .then(function (html) {
        ...
        if (grid && live.length) grid.innerHTML = live.slice(0, 2).map(epCard).join('');
